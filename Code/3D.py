...
for f in range(len(folders)):
    ...
    summationMedia = []
    summationVarianza = []
    text_file = open("./../Output/" + folders[f] + "/3DDistancesMediaVarianza.txt", "w")
    ...
    text_file = open("./../Output/" + folders[f] + "/3DDistancesTraveled.txt", "w")
    for i in range(50):
        text_file.write(str(i) + " - " +str(distancesTraveled[i]) + "\n")
    text_file.close()

    # Calcolo il risparmio di spazio
    text_file = open("./../Output/" + folders[f] + "/3DDimensionSave.txt", "w")
